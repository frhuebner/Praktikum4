...
\begin{figure}[h]
  ...
  \begin{subfigure}[h]{0.5\textwidth}
    \centering
  \includegraphics[width=\textwidth]{Fotos/verteilung_sekundaer.png}
  \subcaption{Teilchenzahl in Abhängigkeit der Höhe, aus \cite{Agashe:2014kda}}
  \label{fig:fluss}
  \end{subfigure}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \foreach \x in {0.25,0.5,0.75,1}{
       \draw (-0.5,\x*5)--(-0.6,\x*5) node [left] {$\x$};
    }
     \end{tikzpicture}
  \caption{Energieverlust eines Myons in Polystyrol}
  \label{fig:landau}
