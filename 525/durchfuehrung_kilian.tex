...
Nun wird die Verzögerung so eingestellt, dass beide Signale in synchron sind. Außerdem wird mit dem GDG die Breite des SCA-Pulses so eingestellt, dass das analoge Signal innerhalb des Pulses liegt (Abbildung \ref{fig:zeitliche_breite_sca_und_analog}).
...
Die Auflösungszeit, die angibt, wie viel Zeit zwischen zwei Ereignissen vergehen muss, damit sie als getrennt erkannt werden, ist durch die Dauer der SCA-Signale gegeben. In Abbildung \ref{fig:slow_koinzidenz} lässt sich die Auflösungszeit zu etwa \SI{0.55}{\micro\second} ablesen. 
...
Nun muss noch die zeitliche Koinzidenz der beiden Kreise eingestellt werden. Dazu werden das Koinzidenzsignal vom Slow-Kreis und das Signal des TAC auf dem Oszilloskop beobachtet. Mit Hilfe des GDG werden die Signale synchronisiert (Abbildung \ref{fig:fast_und_slow}). \newpage
\begin{figure}[h]
  \centering
  \begin{tikzpicture}
